...
FRAMES_DIR = Path(os.getenv("FRAMES_DIR", Path.cwd() / "sessions" / "current" / "frames"))
...
def test_generate_sidecars_for_all_frames(tmp_path: Path):
    ...
    for frame in frames:
        ...
        if sidecar.exists():
            sidecar.unlink()
        service.process_frame(frame, stats)
...
def test_idempotent_processing():
    ...
    for frame in frames:
        service.process_frame(frame, stats)
    mtimes_first = {f.with_suffix(".detections.json"): f.with_suffix(".detections.json").stat().st_mtime for f in frames}
    # Second pass (should be no modification since sidecars exist)
    for frame in frames:
        service.process_frame(frame, stats)
    mtimes_second = {p: p.stat().st_mtime for p in mtimes_first.keys()}
    assert mtimes_first == mtimes_second, "Sidecar files modified on second processing pass"
